<template>
  <div>
    ...
    <el-card>
      ...
      <el-row :gutter="100">
        <el-col :span="12">
          <el-input placeholder="输入用户手机号搜索" v-model="queryInfo.query" clearable @clear="getAddressList">
      ...
      <el-table :data="addresslist" border stripe>
        ...
        <el-table-column align="center" sortable prop="name" label="通讯录姓名"></el-table-column>
        <el-table-column align="center" sortable prop="phone" label="通讯录手机号"></el-table-column>

        <el-table-column align="center" label="操作">
          <template slot-scope="scope">
            <el-button type="danger" icon="el-icon-delete" size="mini" circle @click="removeAddressById(scope.row.id)"></el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- 分页区域 -->
  ...
  export default {
    ...
    methods: {
      async getAddressList() {
        ...
        if (res.status !== 200) {
          return this.$message.error('获取用户通讯录失败！')
        }
        this.addresslist = res.addressListEntityList;
      ...
      async removeAddressById(id) {
        ...
        const {
          data: res
        } = await this.$http.delete('deleteAddress/' + id)
