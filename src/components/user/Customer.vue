  ...
  <div>
    ...
    <el-card>
      ...
      <el-table :data="userlist" border stripe>
        ...
        <el-table-column align="center" sortable prop="email" label="验证邮箱"></el-table-column>
      ...
      <el-dialog title="修改用户信息" :visible.sync="editDialogVisible" width="50%" @close="editDialogClosed">
        <!-- 内容主体 -->
        <el-form :model="editUserForm" ref="editUserFormRef" :rules="editUserFormRules" label-width="80px">
          <el-form-item label="用户名" prop="username">
          ...
          <el-form-item label="验证邮箱" prop="email">
            <el-input v-model="editUserForm.email" placeholder="用户修改密码用"></el-input>
          </el-form-item>
        </el-form>
        <span slot="footer" class="dialog-footer">
  ...
  export default {
    ...
    methods: {
      ...
      async showEditDialog(id) {
        const {
          data: res
        } = await this.$http.get('editCustomer/' + id)
        if (res.status !== 200) {
          return this.$message.error('查询用户信息失败！')
        }
        this.editUserForm = res.customerEntity
      ...
      editUser() {
        ...
        this.$refs.editUserFormRef.validate(async valid => {
          ...
          const {
            ...
            'editCustomer/' + this.editUserForm.id + '/' +
          ...
          if(res==501)
          {
            return this.$message.error('该用户手机号已存在！')
          }
      ...
      async removeUserById(id) {
        ...
        const {
          data: res
        } = await this.$http.delete('editCustomer/' + id)
      ...
      addUser() {
        ...
        this.$refs.addUserFormRef.validate(async valid => {
          ...
          const {
            data: res
          } = await this.$http.post('addCustomer', this.addUserForm)
          if (res == 501) {
            return this.$message.error('该用户手机号重复！')
          }
          else
          {
            if(res!=201)
